...
| "Browse All" grids (alphabetical) | Complete | Not implemented | Fire TV grids show the originating shelf/category title at the top (#353), matching Movies/Shows/Music page headers. Roku shelves are capped at 10 items with no overflow grid |
...
| Watch state persistence (95% threshold) | Complete | Deviation | Fire TV snapshots show/season/episode and falls back to that identity after a replacement encode, with foreground-loss flushing and newest-write-wins ordering. Roku's `Registry.SetWatchState`/`GetWatchState` (#379) snapshot the same normalized-title/season/episode identity and fall back to it when the fingerprint alone misses, so an overnight recode doesn't strand progress even though Roku still has no season/episode navigation UI (it always plays a show's first episode, now resuming it via this fallback) and is **LRU-capped at 500 entries** (registry size limit) vs. Fire TV's unbounded SharedPreferences store |
